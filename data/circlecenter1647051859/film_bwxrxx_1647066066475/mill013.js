...
const pigmentchoice = (process.argv[2] || "bwxrxx");
const text = (process.argv[3] || "");
const text2 = (process.argv[4] || "");
const isendfilm = (process.argv[5] || false);
console.log(`pigmentchoice=${pigmentchoice}, text=${text}, text2=${text2}`);
// const pigmentchoice = (process.argv[2] || "bwxrxx");
const prefix = "film";
...
if(isendfilm) {
let createfilmfile = "createfilm_seq_"+timestamp+".sh";
let createfilm = `
...
`;
fs.writeFileSync(createfilmfile, createfilm, (err) => {
  if (err)
    console.log(err);
  else {
    console.log("File written successfully\n");
